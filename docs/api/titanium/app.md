...
`Titanium.App` module, see [Titanium.App.Properties](Titanium.App.Properties).
...
These events are fired using [fireSystemEvent](Titanium.App.fireSystemEvent), which is available in Titanium Mobile 3.0.0.

* [EVENT_ACCESSIBILITY_ANNOUNCEMENT](Titanium.App.EVENT_ACCESSIBILITY_ANNOUNCEMENT), available in iOS and Android, asks the device's
accessibility system to make an announcement. The announcement itself is a string passed as the second
argument to [fireSystemEvent](Titanium.App.fireSystemEvent). *How* the device's accessibility actually makes that announcement
...
* [EVENT_ACCESSIBILITY_LAYOUT_CHANGED](Titanium.App.iOS.EVENT_ACCESSIBILITY_LAYOUT_CHANGED), available only on iOS, can be used to alert
...
No second parameter is required when firing this event with [fireSystemEvent](Titanium.App.fireSystemEvent).

* [EVENT_ACCESSIBILITY_SCREEN_CHANGED](Titanium.App.iOS.EVENT_ACCESSIBILITY_SCREEN_CHANGED), available only on iOS, can be used to alert
...
[addEventListener](Titanium.App.addEventListener), namely [accessibilitychanged](Titanium.App.accessibilitychanged).  This event, available on iOS and on Android
