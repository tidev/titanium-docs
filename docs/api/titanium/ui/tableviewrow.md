...
Note that a table-level `rowHeight` or [minRowHeight](Titanium.UI.TableView.minRowHeight) acts as a
lower bound and is still applied to a `0` height, so the row will not collapse when either is set.
