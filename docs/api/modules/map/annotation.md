...
* To display an image, set the [leftButton](Modules.Map.Annotation.leftButton) or
  [rightButton](Modules.Map.Annotation.rightButton) property to an image URL. (On
  iOS, you can also use a [SystemButton](Titanium.UI.iOS.SystemButton) constant
  to use one of the native system button icons.)

* To add another type of view to the annotation, set the
  [leftView](Modules.Map.Annotation.leftView) or
  [rightView](Modules.Map.Annotation.rightView) property to a [View](Titanium.UI.View)
  object.
